// ===============================================
// 🜂 Shadow Realms API — CommonJS Production Final
// Version: v6.final
// Monolithic index.js — NO DDL, NO ALTER, NO MIGRATIONS
// ===============================================
...
// ---------- Root & health ----------
app.get('/', (req, res) => res.send(`${APP_NAME} ${APP_VERSION} — OK`));
app.get('/api/health', (req, res) => res.json({ status: 'ok', app: APP_NAME, version: APP_VERSION, ts: new Date().toISOString() }));

// ===================================================================
// 1) CORE: Players / registration / lookup
// ===================================================================
app.post('/api/register', async (req, res) => {
  try {
    ...
    // Insert or update player record. Assumes players table exists and has uuid unique constraint.
    await run(
...
// ===================================================================
// 2) ECONOMY: gold updates, get balance, shop purchase
// ===================================================================
...
// transactional shop purchase (deduct gold and record sale)
...
// ===================================================================
// 6) MODULES: pack xp, faerie diplomacy, warlock pacts, celestial vault
// ===================================================================
...
// ===================================================================
// 7) HUNTER TRIALS / PORTALS / WORLD EVENTS / EXPOSURE
// ===================================================================
...
app.post('/api/world/exposure', async (req, res) => {
  try {
    ...
    // insert-or-update like behaviour but no DDL
...
// ===================================================================
// 8) BONDS / BLOODLINES / ROMANCE / FAMILY (full romance module)
// ===================================================================

// create generic bond (parabatai, oath, family, romantic)
...
app.get('/api/bloodlines', async (req, res) => {
  try {
    const r = await run('SELECT * FROM bloodlines ORDER BY family_name');
...
// Romance relationships (hetero, gay, lesbian handled by relationship_type field)
...
// affinity points between partners
...
// adoption into bloodline / family children
app.post('/api/romance/adopt', async (req, res) => {
  try {
    ...
    // ensure bloodline exists (no DDL here; if missing table/column you'll see the SQL error)
    const bl = await run('SELECT * FROM bloodlines WHERE family_name = $1', [bloodline_name]);
    if (bl.rowCount === 0) {
      // create record if table permits - this is a normal INSERT (not ALTER)
...
// ===================================================================
// 12) ADMIN (dangerous) — NO AUTH included, protect in production
// ===================================================================
app.post('/api/admin/reset_all', async (req, res) => {
  try {
    ...
    for (const t of tables) {
      try { await run(`DELETE FROM ${t}`); } catch (e) { /* ignore missing */ }
    }
    res.json({ status: 'ok', message: 'attempted to clear core tables' });
  } catch (err) {
    respondError(res, err);
  }
});
...
app.get('/api/version', (req, res) => res.json({ app: APP_NAME, version: APP_VERSION }));
