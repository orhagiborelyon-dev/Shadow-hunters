// ===============================================
// 🜂 Shadow Realms API — CommonJS Monolith
// Version 6.4 — "Heavenly Codex Final"
// Copy-paste as index.js (Node.js 18+)
// ===============================================
...
app.post("/api/player/update", async (req, res) => {
  try {
    ...
    await run(`UPDATE players SET ${setParts} WHERE uuid = $1`, params);
...
// ----------------------
// 6) Module endpoints: pack xp, faerie diplomacy, warlock pacts, celestial vault
// ----------------------
...
// bloodlines (uses 'bloodlines' table)
...
// ----------------------
// 12) Admin (dangerous) + Utilities
// ----------------------
app.post("/api/admin/reset_all", async (req, res) => {
  try {
    // Dangerous: clears many tables — protect in production
    const tables = [
    ...
    for (const t of tables) {
      try { await run(`DELETE FROM ${t}`); } catch (e) { /* ignore missing tables */ }
    }
